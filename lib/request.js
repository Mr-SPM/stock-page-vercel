...
export async function getStockList(datas) {
    ...
    try {
        while (stockList.length > 0) {
            ...
            requestList.push(getList(part))
        }
        resList = (await Promise.all(requestList)).flat()
        console.log('抓取完毕', resList)
        return resList
    }
    catch (err) {
        console.warn(err)
...
export async function getOnlineStockList(type = 'SH') {
    ...
    array = firstData.diff.filter(item => !/st|退/i.test(item.f14) && item.f2 !== '-').map(mapFC)
    for (let i = pageNo; i < pages; i++) {
        const res = await getStockPage(i + 1, type);
        array = array.concat(res.diff.filter(item => !/st|退/i.test(item.f14) && item.f2 !== '-').map(mapFC))
    }
    console.log(`加载 ${type} ${array.length}`)
    return array
